...
var on_mouse_up_connection = null;

dojo.addOnLoad(function() {
	dicole.register_template("annotation_bubble", {templatePath: dojo.moduleUrl("dicole.wiki", "annotation_bubble.html")});
	dicole.register_template("annotation_bubble_invalid", {templatePath: dojo.moduleUrl("dicole.wiki", "annotation_bubble_invalid.html")});
	dicole.register_template("annotation_bubble_confirm", {templatePath: dojo.moduleUrl("dicole.wiki", "annotation_bubble_confirm.html")});
	dicole.register_template("annotation_comments_box", {templatePath: dojo.moduleUrl("dicole.wiki", "annotation_comments_box.html")});
	dicole.register_template("annotation_comments", {templatePath: dojo.moduleUrl("dicole.wiki", "annotation_comments.html")});

	connect_mouseup();

    dojo.query("#new_search_input").forEach(function( input ) {
	...
	dojo.query(".wiki_anno_comment_link").forEach(function(element) {
        var json;
        try {
    		json = dojo.fromJson(dojo.attr(element, "title"));
        }
        catch ( e ) {
            return;
        }
		dojo.attr(element, "title", dicole.msg("Show comments"));
	
		var annotation_id = element.id.split("_")[4];
		var id = "annotation_comments_box_" + annotation_id;
		dojo.place(dicole.process_template("annotation_comments_box", {"id": id}), dojo.body());
		
		var box = dojo.byId(id);
		process_comments_box(box);

		var button = dojo.query(".annotation_comment_button", box)[0];
		var textarea = dojo.query(".annotation_comment_textarea", box)[0];
		var container = dojo.query(".annotation_comments_container", box)[0];
		var close_button = dojo.query(".annotation_close", box)[0];
		var arrow = dojo.query(".annotation_comments_arrow", box)[0];
		
		dojo.connect(element, "onclick", null, function(event) {
			dojo.stopEvent(event);
			dojo.disconnect(on_mouse_up_connection);
			dojo.addClass(element, "open");
			
			var wiki_coordinates = dojo.coords(dojo.byId("wiki_block_1"), true);
			var element_coordinates = dojo.coords(element, true);
			dojo.marginBox(arrow, {"l": element_coordinates.l - 10});
			dojo.marginBox(box, {
				"t": element_coordinates.y + element_coordinates.h + 5,
				"l": wiki_coordinates.x,
				"w": wiki_coordinates.w
			});
			dojo.place(box, dojo.body());
			dojo.style(box, "display", "block");
			
			load_comments(element, container, json.data_url);
		});
		
		var annotations = dojo.query(".wiki_anno_" + annotation_id);
		dojo.connect(element, "onmouseover", null, function(event) {
			annotations.addClass("annotation_highlight");
		});
		dojo.connect(element, "onmouseout", null, function(event) {
			annotations.removeClass("annotation_highlight");
		});
		...
		var onclick = function(event) {
			...
			dojo.xhrPost({
				"url": json.add_url,
				"content": {"comment_content": dojo.trim(textarea.value)},
				"handleAs": "json",
				"handle": function(response) {
					container.innerHTML = dicole.process_template("annotation_comments", {"comments": response.comments});
					process_comments(element, container);
					element.innerHTML = response.comment_count;
					textarea.value = textarea.defaultValue;
					dojo.attr(textarea, "rows", 1);
					dojo.style(button, "display", "block");
				}
			});
		};
		
		var onclick_disabled = function(event) {
			dojo.stopEvent(event);
		};
		
		var onclick_connection = dojo.connect(button, "onclick", null, onclick);
		
		var oninput = function(event) {
			if(!textarea.value.length || !dojo.trim(textarea.value).length || textarea.value == textarea.defaultValue) {
				dojo.disconnect(onclick_connection);
				onclick_connection = dojo.connect(button, "onclick", null, onclick_disabled);
				dojo.addClass(button, "button-disabled");
			}
			else {
				dojo.disconnect(onclick_connection);
				onclick_connection = dojo.connect(button, "onclick", null, onclick);
				dojo.removeClass(button, "button-disabled");
			}
			if(event && event.type == "focus") dojo.style(textarea, "color", "inherit");
			else if(event && event.type == "blur" && !textarea.value.length || !dojo.trim(textarea.value).length || textarea.value == textarea.defaultValue) dojo.style(textarea, "color", "#AAA");
		};
		
		dojo.connect(textarea, "onfocus", null, oninput);
		dojo.connect(textarea, "onblur", null, oninput);
		dojo.connect(textarea, "onkeyup", null, oninput);	
		oninput();	
	});
});

function connect_mouseup() {
	if(dicole.get_global_variable("wiki_right_to_annotate")) {
		on_mouse_up_connection = dojo.connect(dojo.body(), "onmouseup", null, on_mouse_up);
	}
}

function load_comments(element, container, data_url) {
	dojo.empty(container);
	dojo.create("div", {"class": "annotation_comments_loading", "innerHTML": dicole.msg("Loading...")}, container);
	dojo.xhrPost({
		"url": data_url,
		"handleAs": "json",
		"handle": function(response) {
...
function process_comments(link_element, container) {
	dojo.query("a.unsupport, a.support, a.publish", container).forEach(function(element) {
		dojo.connect(element, "onclick", null, function(event) {
			dojo.stopEvent(event);
			dojo.empty(container);
			dojo.create("div", {"class": "annotation_comments_loading", "innerHTML": dicole.msg("Loading...")}, container);
			dojo.xhrPost({
				"url": dojo.attr(element, "href"),
				"handleAs": "json",
				"handle": function(response) {
					container.innerHTML = dicole.process_template("annotation_comments", {"comments": response.comments});
					process_comments(link_element, container);
					link_element.innerHTML = response.comment_count;
				}
			});
		});
	});
	dojo.query("a.remove", container).forEach(function(element) {
		dojo.connect(element, "onclick", null, function(event) {
			dojo.stopEvent(event);
			var mouse_coordinates = get_mouse_coordinates(event);
			show_bubble("annotation_bubble_confirm", mouse_coordinates.x, mouse_coordinates.y);
			dojo.connect(dojo.byId("confirm_button"), "onclick", null, function(event) {
				dojo.stopEvent(event);
				dojo.empty(container);
				dojo.create("div", {"class": "annotation_comments_loading", "innerHTML": dicole.msg("Loading...")}, container);
				dojo.xhrPost({
...
function on_mouse_up(event) {
	var selection = get_selection_object();
	if(is_selection_valid(selection)) {
		var mouse_coordinates = get_mouse_coordinates(event);
		if(!is_child_of_class(selection.startContainer, "wiki_anno") && 
			!is_child_of_class(selection.endContainer, "wiki_anno") && 
			!is_child_of_class(selection.startContainer, "wiki_anno_comment_link") &&
			!is_child_of_class(selection.endContainer, "wiki_anno_comment_link") &&
			!selection_contains_annotation(selection)) {
			var start_block_parent = get_block_parent(selection.startContainer);
			var end_block_parent = get_block_parent(selection.endContainer);
			if(start_block_parent === end_block_parent || 
				selection.startOffset == selection.startContainer.length &&
				selection.endOffset == 0 &&
				get_block_parent_child(selection.startContainer) === start_block_parent.childNodes[start_block_parent.childNodes.length - 1] &&
				get_block_parent_child(selection.endContainer) === end_block_parent.childNodes[0]) {
				show_bubble("annotation_bubble", mouse_coordinates.x, mouse_coordinates.y);
				dojo.connect(dojo.byId("annotate_button"), "onclick", null, on_annotate);
			}
			else show_bubble("annotation_bubble_invalid", mouse_coordinates.x, mouse_coordinates.y);
		}
		else show_bubble("annotation_bubble_invalid", mouse_coordinates.x, mouse_coordinates.y);
	}
	else hide_bubbles();
...
function selection_contains_annotation(selection) {
	var start = get_block_parent_child(selection.startContainer);
	var end = get_block_parent_child(selection.endContainer);
	if(has_next_sibling_with_class(start, "wiki_anno", end) || has_next_sibling_with_class(start, "wiki_anno_comment_link", end)) {
...
function has_next_sibling_with_class(element, css_class, end_element) {
	if(!element.nextSibling || element.nextSibling === end_element) return false;
	if(dojo.hasClass(element.nextSibling, css_class)) return true;
	return has_next_sibling_with_class(element.nextSibling, css_class, end_element);
}

function on_annotate(event) {
	dojo.stopEvent(event);
	...
	var selection = get_selection_object();
	var selection_data = get_selection_data(selection);
	
	var mouse_coordinates = get_mouse_coordinates(event);
	
	dojo.xhrPost({
		"url": dicole.get_global_variable("wiki_start_annotation_url"),
		"content": selection_data,
		"handleAs": "json",
		"handle": function(response) {
			if(response && response.lock_granted) {
				_remove_wiki_controls();
				_hide_sh_switches();
					
				dojo.byId(get_selection_wiki_container(selection).id).innerHTML = response.content;
				
				dojo.place(dicole.process_template("annotation_comments_box", {
					"id": "annotation_comments_box_0",
					"cancel_url": dicole.get_global_variable("wiki_cancel_annotation_url") + "?edit_lock=" + response.lock_id
				}), dojo.body());
				
				var box = dojo.byId("annotation_comments_box_0");
				var button = dojo.query(".annotation_comment_button", box)[0];
				var textarea = dojo.query(".annotation_comment_textarea", box)[0];
				var arrow = dojo.query(".annotation_comments_arrow", box)[0];
				...
				var onclick = function(event) {
					...
					selection_data["edit_lock"] = response.lock_id;
					selection_data["comment_content"] = dojo.trim(textarea.value);
					
					dojo.xhrPost({
						"url": dicole.get_global_variable("wiki_save_annotation_url"),
						"content": selection_data,
						"handleAs": "json",
				...
				var oninput = function(event) {
					...
					if(event && event.type == "focus") dojo.style(textarea, "color", "inherit");
					else if(event && event.type == "blur") dojo.style(textarea, "color", "#AAA");
				};
				...
				dojo.style(arrow, "display", "none");
				var wiki_coordinates = dojo.coords(dojo.byId("wiki_block_1"), true);
				dojo.marginBox(box, {
					"t": mouse_coordinates.y + 15,
					"l": wiki_coordinates.x,
					"w": wiki_coordinates.w
				});
				dojo.place(box, dojo.body());
...
function process_comments_box(box) {
	if (!dicole.get_global_variable("wiki_right_to_annotate")) {
		dojo.query(".annotation_comment_textarea, .annotation_comment_button", box).forEach(function(element) {
			dojo.style(element, {"display": "none"});
		});
	}
	var annotation_comments_textarea = dojo.query(".annotation_comment_textarea", box)[0];
	var default_value = annotation_comments_textarea.value;
	dojo.connect(annotation_comments_textarea, "onfocus", null, function(event) {
		if(annotation_comments_textarea.value == default_value) {
			annotation_comments_textarea.value = "";
			dojo.attr(annotation_comments_textarea, "rows", 1);
		}
	});
	dojo.connect(annotation_comments_textarea, "onblur", null, function(event) {
		if(!annotation_comments_textarea.value.length) {
			annotation_comments_textarea.value = default_value;
			dojo.attr(annotation_comments_textarea, "rows", 1);
		}
	});
	dojo.connect(annotation_comments_textarea, "onkeyup", null, dojo.partial(autoresize_textarea, annotation_comments_textarea));
	dojo.connect(annotation_comments_textarea, "onkeydown", null, dojo.partial(autoresize_textarea, annotation_comments_textarea));
}
...
function get_selection_data(selection) {
	var selection_wiki_container = get_selection_wiki_container(selection);
	var selection_block_parent = get_block_parent(selection.startContainer);
	var selection_start_root = get_block_parent_child(selection.startContainer);
	var selection_end_root = get_block_parent_child(selection.endContainer);
	
	var before = function(node, escaped) {
		if(node.parentNode === selection_wiki_container) {
			if(node === selection_block_parent || dojo.hasClass(node, "wiki_controls") || node.id == "toc_header" || node.id == "toc_block") {
				if(node.previousSibling) return arguments.callee(node.previousSibling, true);
				return "";
			}
			if(node.previousSibling) return arguments.callee(node.previousSibling, true) + (escaped ? get_element_text(node) : "");
			return (escaped ? get_element_text(node) : "");
		}
		var previous_node = node.previousSibling ? node.previousSibling : node.parentNode;
		if(node === selection.startContainer) {
			return arguments.callee(previous_node, node.parentNode === selection_block_parent && node === selection_start_root) + (node.nodeName == "#text" ? node.nodeValue.substring(0, selection.startOffset) : "");
		}
		if(node.parentNode === selection_block_parent) {
			if(!escaped) return arguments.callee(previous_node, node === selection_start_root) + (node.nodeName == "#text" ? node.nodeValue : "");
			return arguments.callee(previous_node, true) + get_element_text(node);
		}
		if(node === selection_block_parent) {
			if(node.previousSibling) return arguments.callee(node.previousSibling, true);
			return arguments.callee(node.parentNode, false);
		}
		return arguments.callee(previous_node, node.previousSibling) + (escaped ? get_element_text(node) : (node.nodeName == "#text" ? node.nodeValue : ""));
	}(selection.startContainer, false);
	
	var after = function(node, escaped) {
		if(node.parentNode === selection_wiki_container) {
			if(node === selection_block_parent || dojo.hasClass(node, "wiki_controls") || node.id == "toc_header" || node.id == "toc_block") {
				if(node.nextSibling) return arguments.callee(node.nextSibling, true);
				return "";
			}
			if(node.nextSibling) return (escaped ? get_element_text(node) : "") + arguments.callee(node.nextSibling, true);
			return (escaped ? get_element_text(node) : "");
		}
		var next_node = node.nextSibling ? node.nextSibling : node.parentNode;
		if(node === selection.endContainer) {
			return (node.nodeName == "#text" ? node.nodeValue.substring(selection.endOffset) : "") + arguments.callee(next_node, node.parentNode === selection_block_parent && node === selection_end_root);
		}
		if(node.parentNode === selection_block_parent) {
			if(!escaped) return (node.nodeName == "#text" ? node.nodeValue : "") + arguments.callee(next_node, node === selection_end_root);
			return get_element_text(node) + arguments.callee(next_node, true);
		}
		if(node === selection_block_parent) {
			if(node.nextSibling) return arguments.callee(node.nextSibling, true);
			return arguments.callee(node.parentNode, false);
		}
		return (escaped ? get_element_text(node) : (node.nodeName == "#text" ? node.nodeValue : "")) + arguments.callee(next_node, node.nextSibling);
	}(selection.endContainer, false);

	return {
		"page_id" : dicole.get_global_variable("wiki_page_id"),
		"base_version_number" : dicole.get_global_variable("wiki_base_version_number"),
		"edit_target_id": selection_wiki_container.id.toString().split("_")[2],
		"edit_target_type": "annotation",
		"before": before,
...
function is_selection_valid(selection) {
	if(
		selection && 
		selection.toString && 
		selection.toString().length && 
		!is_child_of_class(selection.startContainer, "annotation_comments") && 
		!is_child_of_class(selection.endContainer, "annotation_comments") &&
		is_child_of_id(selection.startContainer, "wiki_block_1") && 
		is_child_of_id(selection.endContainer, "wiki_block_1")) return true;
	return false;
}

function get_selection_wiki_container(selection) {
	return get_element_parent_until_class(selection.startContainer, "wiki_content_container");
}
...
function is_block_element(element) {
	return !in_array(inline_elements, element.tagName.toLowerCase());
}

function get_block_parent(element) {
	if(is_block_element(element.parentNode)) return element.parentNode;
	return get_block_parent(element.parentNode);
}

function get_block_parent_child(element) {
	if(is_block_element(element.parentNode)) return element;
	return get_block_parent_child(element.parentNode);
}

function get_element_parent_until_class(element, css_class) {
	if(dojo.hasClass(element.parentNode, css_class)) return element.parentNode;
	return get_element_parent_until_class(element.parentNode, css_class);
}

function is_child_of_id(element, id) {
	if(!element.parentNode) return false;
	if(element.parentNode.id == id) return true;
	return is_child_of_id(element.parentNode, id);
}

function is_child_of_class(element, css_class) {
	if(!element.parentNode) return false;
	if(dojo.hasClass(element.parentNode, css_class)) return true;
	return is_child_of_class(element.parentNode, css_class);
}

function show_bubble(type, left, top) {
	dojo.place(dicole.process_template(type, {}), dojo.body());
	var bubble = dojo.byId(type);
	dojo.query(".annotation_close", bubble).forEach(function(element) {
		dojo.connect(element, "onclick", null, function(event) {
			dojo.stopEvent(event);
			hide_bubbles();
		});
	});
	var bubble_dimensions = dojo.marginBox(bubble);
	dojo.marginBox(bubble, {
		"l": left + 20, 
		"t": top - 10
	});
}
...
function hide_bubbles() {
	dojo.query(".bubble").forEach(function(bubble) {
		dojo.destroy(bubble);
	});
}
...
window.onbeforeunload = function() {
    if ( document.forms["Form"] && document.forms["Form"].edit_lock && document.forms["Form"].edit_lock.value && ! document.forms["Form"].cancel.value == 1 &&
