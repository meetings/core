// [% # $Id: summarybox.uncompressed.js,v 1.2 2007-08-24 22:46:47 amv Exp $ # %]
...
function getColumns(parentObj) {
	...
	if (parent.childNodes.length) {
        ...
        for ( i = 0; i <  parent.childNodes.length; i++ ) {
            ...
            if ( /td/i.test( child.nodeName ) ) columns.push( child );
        }
...
function getBoxInfo(object) {
	...
	for (i=0; i<cols.cols.length; i++) {
		if(box.parentNode == cols.cols[i]) { var x = i; break; }
	}
	for (i=0; i<rows.rows.length; i++) {
		if(box == rows.rows[i]) { var y = i; break;}
	}
	return { cols:cols,rows:rows,box:box,id:id,parent:parent,x:x,y:y }
}
