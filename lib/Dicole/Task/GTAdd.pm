...
Dicole::Task::GTAdd->mk_accessors( qw(
  box_x 
  box_y 
  box_title 
  path_name 
...
sub execute {
    ...
    $self->action->tool->Container->box_at( $x, $y )->add_content(
        $self->action->gtool->get_add
    );

    # Run custom pre-generate tool content operations
    $self->_pre_gen_tool;

    return $self->skip_generate ? 1 : $self->action->generate_tool_content;
}

sub _pre_init {
    my ( $self )  = @_;
    if ( CTX->request->param( 'preview_add' ) ) {
        $self->_tool_config( 'rows', 2 );
    }
}
