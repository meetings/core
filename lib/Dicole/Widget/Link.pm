...
sub DEFAULT_TEMPLATE { 'widget_link' };

sub ACCESSORS { (
    href => Dicole::Widget::ACCESSOR_RAW,
    media => Dicole::Widget::ACCESSOR_RAW,
    rel => Dicole::Widget::ACCESSOR_RAW,
    type => Dicole::Widget::ACCESSOR_RAW,
...
sub template_params {
    ...
    if ( $href && $file_version && ! ( $href =~ /\&v\=|\?v\=/ || $href =~ /^http/ ) ) {
        $params->{href} = $href . ( ( $href =~ /\?/ ) ? '&v=' . $file_version : '?v=' . $file_version );
    }
...
Dicole::Widget::Link - A class that defines Dicole link widgets
