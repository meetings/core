package Dicole::Action::Common::Edit;

# $Id: Edit.pm,v 1.6 2009-01-07 14:42:32 amv Exp $
...
A common action helper for creating an edit view for an object

=head1 SYNOPSIS

 use base qw( Dicole::Action::Common::Edit );

=head1 DESCRIPTION

A common action helper for implementing an edit view for a SPOPS object. The
edit page has basic input validation, form handling and saves the object.
...
This common generic edit task generates an edit view based on configuration
...
Example configuration:
  ...
  c_box_x = 0
  c_box_y = 0
  c_box_title = Edit users
  c_path_name = Users
  c_skip_security = 1
  c_class = user
  c_preview = 1
  c_cancel_link = /my_action/show?id=IDVALUE
  c_id_param = id
  c_save_redirect = /my_action/show?id=IDVALUE
...
The I<views> property must always be I<edit> while the fields should correspond
...
The task which creates the edit form view. Uses L<Dicole::Generictool> to
...
sub edit {
    ...
    my $id = $self->_pre_init_common_edit;

    # Init tool
    $self->_init_common_edit( $id );

    # Run custom post-init operations
    $self->_post_init_common_edit( $id );

    # Adds some buttons to the view
    $self->_common_buttons_edit( $id );

    # Saves the object if save button pressed
    $self->_common_save_edit( $id );

    return undef if CTX->response->is_redirect;

    my $x = $self->param( 'c_edit' )->{c_box_x} || 0;
    my $y = $self->param( 'c_edit' )->{c_box_y} || 0;
...
=head2 _pre_init_common_edit()

This method is meant to be overridden. It runs before I<_init_common_edit()>.

By default it generates two boxes if the preview button was pressed and
returns the object id being edited based on action parameter I<c_id_param>.
...
sub _pre_init_common_edit {
    ...
    return CTX->request->param( $self->param( 'c_edit' )->{c_id_param} );
}

=pod

=head2 _post_init_common_edit( ID )

This method is meant to be overridden. It runs after I<_init_common_edit()>.

By default, it adds a new path segment based on the action parameter
I<c_path_name>.

Receives the object id currently under editing as a parameter.
...
sub _post_init_common_edit {
    ...
    $self->tool->Path->add( name => $self->_msg(
        $self->param( 'c_edit' )->{c_path_name}
    ) ) if $self->param( 'c_edit' )->{c_path_name};

}

=pod

=head2 _gen_preview_box_edit()
...
=head2 _common_buttons_edit( ID )

This method is meant to be overridden. It runs after
I<_post_init_common_edit()>.
...
Receives the object id currently under edit as a parameter.
...
=head2 _pre_save_edit( DATA )

This method is meant to be overridden. It runs after I<_validate_input_edit()>
...
Remember to return undef if you want to use the default message.
...
Receives the id of the object currently under edit.
...
sub _common_save_edit {
    my ( $self, $id ) = @_;

    if ( CTX->request->param( 'save' ) ) {

        # Validate input parameters
        my ( $code, $message ) = $self->_validate_input_edit( $id );
        ...
        if ( $code ) {
            ...
            if ( $self->_pre_save_edit( $data ) ) {

                $data->data_save;

                # Run post tasks for saving the object
                my $new_message = $self->_post_save_edit( $data );
                $new_message ||= $message;
                $self->tool->add_message( $code, $new_message );

                CTX->response->redirect(
                    $self->_replace_tags_in_link(
                        $self->param( 'c_edit')->{c_save_redirect}, $id
                    )
                ) if $self->param( 'c_edit')->{c_save_redirect};
            }
        } else {
...
=head2 _pre_gen_tool_edit()

This method is meant to be overridden. It is run before
the tool is generated but just after the list box is
created.

Hint: this is a good place to place a new box just after the edit box.

=cut

sub _pre_gen_tool_edit {
    my ( $self )  = @_;
}
...
=head2 _config_tool_edit( [KEY, VALUE] )
...
I<_post_init_tool_edit()> method.

=cut

sub _config_tool_edit {
    my ( $self, $key, $value ) = @_;
    unless ( ref( $self->{_config_tool_edit} ) eq 'HASH' ) {
        $self->{_config_tool_edit} = {};
    }
    if ( $key ) {
        $self->{_config_tool_edit}{$key} = $value;
    }
    return $self->{_config_tool_edit};
}

=pod

=head2 _init_common_edit()
...
sub _init_common_edit {
    my ( $self, $id ) = @_;
    $self->_init_common_tool( {
        tool_config => $self->_config_tool_edit,
        class => $self->param( 'c_edit' )->{c_class},
        skip_security => $self->param( 'c_edit' )->{c_skip_security},
